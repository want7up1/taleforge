...
4. 遵守已确立事实：人物、物品、地点、时间线不凭空变化。
5. 当前幕的必需锚点未在剧情中真实发生前，不进入下一幕；全部达成后随玩家行动自然收束转场。
6. 受控 Markdown：正文用自然段；\`### 场景名\` 仅在地点或时间明显切换时使用；\`**重点**\` 仅标记真正关键的线索或异常，宁缺毋滥；\`>\` 引用块仅用于信件、公告、录音等剧情内文本载体；不使用表格、代码块、H1/H2。`
...
玩家发来第一条消息时（无论内容是什么），以上面的开场场景开篇，收在开场钩子上，并给出第一组行动选项。
`
}
